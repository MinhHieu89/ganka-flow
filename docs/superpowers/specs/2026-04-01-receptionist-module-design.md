...
Any status can transition to Đã hủy (cancelled).
```
...
| Đã hủy | Gray (#6b7280) | Cancelled visit (from any status) |
...
- Time slots in the appointment form use simplified placeholder ranges (08:00–19:30, 30-min intervals). Real clinic hours vary by day of week — backend will enforce actual availability later.
- Slot capacity is hardcoded at 2 BN/slot for the mockup. Configurable in backend.
